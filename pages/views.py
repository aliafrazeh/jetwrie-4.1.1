...
class PostDetail(LoginRequiredMixin,DetailView):
	...
	def get_object(self):
		...
		if article.private and self.request.user == article.author:
			return article
		else:
...
def tickView(request):
	...
	return render(request,'pages/tick.html',{'link':link})

def chat_ai_jetwrie(request):
	msg = request.POST.get('input')
	response = " "
	if msg == "سلام":
		response = "سلام خوبی چطور می تونم کمکت کنم؟"
	if msg == "خوبی":
		response = "ممنون خوبم چطور میتونم کمکت کنم؟"
	if msg == "تو کی هستی؟":
		response = "من آریالا هوش مصنوعی هستم و اینجا در جت رای با شما حرف می زنم!"


	
	return render(request,'pages/chatbot.html',{"response":response})
